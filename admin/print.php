...
@page { margin: 0; }
	...
	while($row=mysqli_fetch_array($re))
	{
		...
		$days = $row['noofdays'];
									...
									$type_of_gown = 0;       
									if($tgown=="Superior Room")
									{
										$type_of_gown = 320;
									
									}
									else if($tgown=="Deluxe Room")
									{
										$type_of_gown  = 220;
									}
									else if($tgown=="Guest House")
									{
										$type_of_gown  = 180;
									}
									else if($tgown=="Single Room")
									{
										$type_of_gown  = 150;
									}
									...
									if($bed=="Single")
									{
										$type_of_gown = $type_of_gown  * 1/100;
									}
									else if($bed=="Double")
									{
										$type_of_gown  = $type_of_gown  * 2/100;
									}
									else if($bed=="Triple")
									{
										$type_of_gown  = $type_of_gown  * 3/100;
									}
									else if($bed=="Quad")
									{
										$type_of_gown  = $type_of_gown  * 4/100;
									}
									else if($bed=="None")
									{
										$type_of_gown  = $type_of_gown  * 0/100;
									}
	...
	?>
		...
		<article>
			...
			<table class="inventory">
				...
				<tbody>
					<tr>
						<td><span ><?php echo $tgown; ?></span></td>
						<td><span ><?php echo $days; ?> </span></td>
						<td><span data-prefix>$</span><span ><?php  echo $type_of_gown;?></span></td>
						<td><span ><?php echo $ngown;?> </span></td>
		...
		<aside>
			...
			<div >
				<p align="center">Email :- info@pwani.com || Web :- www.pwani.com || Phone :- +254 727 901 740 </p>
			</div>
		</aside>
	</body>
</html>
...
$free="Free";
$nul = null;
$rpsql = "UPDATE `gown` SET `place`='$free',`cusid`='$nul' where `cusid`='$id'";
if(mysqli_query($con,$rpsql))
{
	$delsql= "DELETE FROM `gownbook` WHERE id='$id' ";
